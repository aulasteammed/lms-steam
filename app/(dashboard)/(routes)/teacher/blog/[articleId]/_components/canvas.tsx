...
import {
    ...
    CHAR_LIMITS, TEMPLATE_LABELS, SocialIcon, charColor,
} from "./types";

// ─── Editable field ───────────────────────────────────────────────────────────

// Shared editable div — syncs DOM when value changes externally (e.g. from panel)
// isFocused ref prevents cursor-jump while the user is actively typing inside the div
function makeEditableField(placeholderColor: string) {
    return function EditableFieldInner({ value, onChange, placeholder, className, multiline = false }: {
        ...
        // Sync DOM when value changes from outside (panel input)
        useEffect(() => {
            if (!ref.current) return;
            if (isFocused.current) return; // usuario escribiendo aquí, no interrumpir
            // Normalizar antes de comparar para evitar falsos negativos con \n
        ...
        return (
            <div
                ...
                onBlur={() => {
                    ...
                    // Sync value on blur in case panel changed it while focused
                    onChange(ref.current?.innerText ?? "");
                }}
                onCompositionStart={() => { isComposing.current = true; }}
                onCompositionEnd={() => { isComposing.current = false; onChange(ref.current?.innerText ?? ""); }}
                onInput={() => { if (!isComposing.current) onChange(ref.current?.innerText ?? ""); }}
                className={[
    ...
    return (
        <div className={`flex items-start gap-3 py-3 border-t border-b ${border}`}>
            ...
            <div className="w-9 h-9 rounded-full flex-shrink-0 overflow-hidden"
                style={{ background: authorPhoto ? undefined : accent }}>
                {authorPhoto
            ...
            <div className="flex flex-col gap-0.5 min-w-0 flex-1">
                <span className={`text-sm font-semibold leading-tight ${textMain}`}>
                ...
                {authorBio && (
                    <span className={`text-xs leading-relaxed mt-1 ${textSub}`}>{authorBio}</span>
                )}
            </div>
        </div>
    );
...
function BlockToolbar({ blockId, onMove, onRemove }: {
    blockId: string;
...
function ParagraphBlock({ block, onUpdate, onRemove, onMove }: { block: Block } & BlockOps) {
    const editRef     = useRef<HTMLDivElement>(null);
    const isComposing = useRef(false);
    const len         = block.content?.length ?? 0;

    useEffect(() => {
        if (editRef.current) editRef.current.innerText = block.content ?? "";
    // eslint-disable-next-line react-hooks/exhaustive-deps
    }, []);
    ...
    return (
        <div className="relative group border border-transparent hover:border-slate-200 transition-all">
            <BlockToolbar blockId={block.id} onMove={onMove} onRemove={onRemove} />
            <div ref={editRef} contentEditable suppressContentEditableWarning
                data-placeholder="Escribe un párrafo..."
                onCompositionStart={() => { isComposing.current = true; }}
                onCompositionEnd={() => { isComposing.current = false; handleInput(); }}
                onInput={handleInput}
                className="text-sm leading-relaxed text-slate-700 px-1 py-1.5 outline-none min-h-[36px]
...
function PullquoteBlock({ block, accent, onUpdate, onRemove, onMove }: { block: Block; accent: string } & BlockOps) {
    const editRef     = useRef<HTMLDivElement>(null);
    const isComposing = useRef(false);
    const len         = block.content?.length ?? 0;
    ...
    const handleInput = () => {
        if (!isComposing.current) onUpdate(block.id, { content: editRef.current?.innerText ?? "" });
    };
    ...
    return (
        <div className="relative group border border-transparent hover:border-slate-200 transition-all">
            ...
            <div ref={editRef} contentEditable suppressContentEditableWarning
                ...
                onCompositionEnd={() => { isComposing.current = false; handleInput(); }}
                onInput={handleInput}
                className="py-2 px-4 text-base italic text-slate-800 leading-snug outline-none
                           empty:before:content-[attr(data-placeholder)] empty:before:text-slate-300 empty:before:pointer-events-none"
...
function DividerBlock({ block, onRemove, onMove }: { block: Block } & Pick<BlockOps, "onRemove" | "onMove">) {
    return (
...
function ImageBlock({ block, onUpdate, onRemove, onMove }: { block: Block } & BlockOps) {
    ...
    const floatCls = block.imagePosition === "fl"
        ...
        : block.imagePosition === "fr"
            ...
            : "";
    const widthCls = isFloat ? "w-[42%]" : "w-full";
        ...
        <div className={`relative group ${floatCls} ${widthCls}`}>
            ...
            {block.imageUrl ? (
                ...
                <div className={["border border-dashed border-slate-300 bg-slate-50 rounded-sm overflow-hidden", !isFloat ? "aspect-video" : ""].join(" ")}>
                    <FileUpload endpoint="courseImage" action={(url) => { if (url) onUpdate(block.id, { imageUrl: url }); }} />
                </div>
            )}
            ...
            <input value={block.caption ?? ""} onChange={(e) => onUpdate(block.id, { caption: e.target.value })}
                ...
                className="w-full text-xs text-slate-400 px-1 mt-1 outline-none bg-transparent placeholder:text-slate-300 border-b border-transparent focus:border-slate-200 transition-colors" />
        </div>
    );
}
...
export type SharedCanvasProps = {
    ...
    dragHandlers:  { onDragStart: (id: string) => void; onDragOver: (id: string) => void; onDrop: (id: string) => void };
    dragState:     { dragging: string | null; dragOver: string | null };
};

function BlockList({ blocks, accentColor, onBlockUpdate, onBlockRemove, onBlockMove, dragHandlers, dragState }: SharedCanvasProps) {
    const ops: BlockOps = { onUpdate: onBlockUpdate, onRemove: onBlockRemove, onMove: onBlockMove };
    ...
    const renderSolo = (b: Block) => {
        switch (b.type) {
            ...
            case "divider":   return <DividerBlock    key={b.id} block={b} onRemove={ops.onRemove} onMove={ops.onMove} />;
            ...
            {groups.map((group, gi) => {
                if (group.kind === "solo") {
                    return (
                        <div key={group.block.id}
                            draggable
                            onDragStart={() => dragHandlers.onDragStart(group.block.id)}
                            onDragOver={(e) => { e.preventDefault(); dragHandlers.onDragOver(group.block.id); }}
                            onDrop={(e) => { e.preventDefault(); dragHandlers.onDrop(group.block.id); }}
                            className={[
                ...
                return (
                    <div key={`flow-${gi}`} className="block">
                        ...
                        {group.textBlocks.map((tb) => (
                            <ParagraphBlock key={tb.id} block={tb} {...ops} />
                        ))}
...
// ─── Cover image zone (for non-cover templates) ───────────────────────────────

function CoverImageZone({ imageUrl, onUpload }: { imageUrl: string; onUpload: (url: string) => void }) {
    if (imageUrl) {
        return (
...
// ═══════════════════════════════════════════════════════════════════════════════
// COVER PERSON — Rediseño estilo portada de revista
// ═══════════════════════════════════════════════════════════════════════════════

function CanvasCoverPerson({
    title, subtitle, authorName, authorBio, authorPhoto,
    authorSocialPlatform, authorSocialUrl,
    coverImageUrl, accentColor, dark,
    ...
    return (
        <div className="grid" style={{ gridTemplateColumns: "45% 55%", minHeight: 680 }}>
            ...
            <div className="relative flex flex-col justify-between overflow-hidden"
                ...
                {/* Bottom: título + subtítulo */}
                <div className="relative z-10 p-6 pb-8">
                    {/* Línea de acento encima del título */}
            ...
            <div className="flex flex-col" style={{ background: "#fafaf8", padding: "28px 24px" }}>
                ...
                {/* Byline del autor — prominente en la parte superior */}
                <div className="flex items-start gap-3 pb-4 mb-4 border-b border-slate-200">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        {authorBio && (
                            <p className="text-[11px] text-slate-500 leading-relaxed mt-1">{authorBio}</p>
                        )}
                    </div>
                </div>
                ...
                <div className="flex-1">
                    <BlockList {...shared} accentColor={accentColor} />
                </div>

                {/* Upload portada si no existe — zona discreta al fondo */}
                {!coverImageUrl && (
...
function CanvasFullArticle({
    title, subtitle, authorName, authorBio, authorPhoto,
    authorSocialPlatform, authorSocialUrl,
    coverImageUrl, accentColor, onTitleChange, onSubtitleChange, onCoverUpload, ...shared
}: TemplateProps) {
    return (
        <div className="p-8">
            <div className="mb-6"><CoverImageZone imageUrl={coverImageUrl} onUpload={onCoverUpload} /></div>
            <EditableFieldLight value={title} onChange={onTitleChange} placeholder="Título del artículo"
                className="text-3xl font-bold tracking-tight leading-tight text-slate-900 mb-2" />
            <EditableFieldLight value={subtitle} onChange={onSubtitleChange} placeholder="Subtítulo..." multiline
            ...
            <div className="mt-5"><BlockList {...shared} accentColor={accentColor} /></div>
        </div>
    );
}
        ...
        <div className="p-8">
            ...
            <div className="grid gap-5 mb-6 items-start" style={{ gridTemplateColumns: "88px 1fr" }}>
                ...
                <div className="flex flex-col gap-1 pt-1">
                    <EditableFieldLight value={title || authorName} onChange={onTitleChange} placeholder="Nombre del perfil"
                        className="text-2xl font-bold tracking-tight text-slate-900" />
                    <EditableFieldLight value={subtitle} onChange={onSubtitleChange} placeholder="Cargo o descripción..." multiline
...
function CanvasNewsShort({
    ...
    return (
        <div style={{ padding: "36px 48px", maxWidth: 620, margin: "0 auto" }}>
            {!coverImageUrl
                ? <div className="mb-5"><CoverImageZone imageUrl={coverImageUrl} onUpload={onCoverUpload} /></div>
            ...
            <EditableFieldLight value={title} onChange={onTitleChange} placeholder="Título de la noticia"
                className="text-2xl font-bold tracking-tight text-slate-900 mb-1" />
            <div className="h-0.5 w-9 mb-4" style={{ background: accentColor }} />
            <EditableFieldLight value={subtitle} onChange={onSubtitleChange} placeholder="Entradilla breve..." multiline
...
export function ArticleCanvas({ template, darkColor, isSaving, lastSaved, ...props }: CanvasProps) {
    return (
        <main className="flex-1 overflow-y-auto bg-slate-100 flex flex-col items-center px-6 py-5 gap-3">
            ...
            <div className="w-full max-w-[860px] bg-white shadow-lg rounded-sm overflow-hidden">
                {template === "cover_person"   && <CanvasCoverPerson   {...props} dark={darkColor} />}
                {template === "full_article"   && <CanvasFullArticle   {...props} />}
                {template === "profile_simple" && <CanvasProfileSimple {...props} />}
                {template === "news_short"     && <CanvasNewsShort     {...props} />}
            </div>
        </main>
    );
}