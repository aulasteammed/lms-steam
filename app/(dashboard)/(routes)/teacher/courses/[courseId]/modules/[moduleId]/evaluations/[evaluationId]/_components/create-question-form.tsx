...
import { ACTIVE_QUESTION_TYPES, QUESTION_TYPE_LABELS } from "@/lib/evaluation";
...
const defaultAnswersByType = (type: QuestionType): AnswerDraft[] => {
  if (type === "open") {
...
export const CreateQuestionForm = ({
  ...
  const validate = () => {
    ...
    if (questionType !== "open" && validAnswers.length < 2) {
      return "Debes ingresar al menos dos opciones/pasos";
    }
    ...
    if (questionType === "single") {
      ...
      if (correctCount !== 1) return "Single requiere exactamente una opción correcta";
    }
    ...
    <div className="mt-6 border bg-slate-100 rounded-md p-4">
      <div className="font-medium flex items-center justify-between">
        ...
        <Button onClick={() => setIsCreating((current) => !current)} variant="ghost">
          {isCreating ? "Cancelar" : "Añadir pregunta"}
        </Button>
      </div>
