...
import { ACTIVE_QUESTION_TYPES, AppQuestionType, normalizeQuestionType } from "@/lib/evaluation";
...
const validateAnswers = (
  ...
  if (validAnswers.length === 0) {
    return "Debes enviar al menos una respuesta valida";
  }
  ...
  if (type === "single") {
    ...
    if (correct !== 1) return "Single requiere exactamente una opcion correcta";
  }
...
export async function PATCH(
  ...
  try {
    ...
    const questionType = type as AppQuestionType;

    const validationError = validateAnswers(questionType, normalizedAnswers);
    if (validationError) {
